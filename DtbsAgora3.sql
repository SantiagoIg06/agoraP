create database agora3;
use agora3;


create table rol (id int (11) primary key auto_increment, rol varchar (10) );
create table usuario (id int primary key auto_increment, nombre varchar(45) not null, apellido varchar (45) not null, correo varchar (100) not null, contraseña varchar (10) not null, fecha_nacimiento date, idRol int (11),constraint fk_rol foreign key (idRol) references rol (id));
...
create table empresa (id int primary key auto_increment,nom_empresa varchar (45)not null,idUsu int,idDireccion int,idNumMovilEmp int, constraint fk_Direccion foreign key (idDireccion) references Direccion (id),constraint fk_usuEMP foreign key (idUsu) references usuario (id),constraint fk_numMovEmp foreign key (idNumMovilEmp) references num_movil (id));
...
CREATE VIEW detalles_usuario AS
...
DELIMITER ;
CALL InsertarEmpresa('Nueva Empresa', 8, 3, 4);
